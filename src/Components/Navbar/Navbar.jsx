...
export default function Navbar() {
  ...
  function NavigateToFacebook() {
    window.open(
      "https://www.facebook.com/profile.php?id=100083159863287",
      "_blank"
    );
  }
  ...
  function NavigateToLinkedIn() {
    window.open(
      "https://www.linkedin.com/in/abdelwahab-mohamed-953904269/",
      "_blank"
    );
  }

  function NavigateToGitHub() {
    window.open("https://github.com/AbdelwahabEladawy", "_blank");
  }

  return (
    <header
      className={`w-full fixed overflow-x-hidden z-50 ${
        isDarkMode ? "bg-background-primary" : "bg-background-secondary"
      }`}
    >
        ...
        <div className="flex items-center justify-between flex-wrap">
          <div className="flex items-center flex-shrink-0">
            <div className="text-white font-bold text-xl flex items-center">
              <svg
                className="text-main-theme w-5 h-5"
              ...
              <Link className="text-main-theme me-8" to="/">
                FreshCart
              </Link>
            </div>
            {userToken && (
                  ...
                  <li>
                    <NavLink
                      ...
                      className={`block px-1 py-2 ${
                        isDarkMode ? "text-white" : "text-secondary-theme"
                      } font-semibold`}
                    >
                      Home
                    </NavLink>
          ...
          <div className="flex gap-2 items-center flex-shrink-0">
            <div className="social-media flex items-center">
              <i
                ...
                className={`fa-brands mx-1 fa-facebook-f cursor-pointer ${
                  isDarkMode ? "text-white" : "text-secondary-theme"
                }`}
              ></i>
              <i
                onClick={NavigateToLinkedIn}
                className={`fa-brands mx-1 fa-linkedin cursor-pointer ${
                  isDarkMode ? "text-white" : "text-secondary-theme"
                }`}
              ></i>
              <i
                onClick={NavigateToGitHub}
                className={`fa-brands mx-1 fa-github cursor-pointer ${
                  isDarkMode ? "text-white" : "text-secondary-theme"
                }`}
              ></i>
              <NavLink to="/cart" className="relative flex items-center ml-4">
                <svg
                  className="w-6 h-6 inline-block text-black dark:text-white"
                  fill="none"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
              ...
              <ul className="flex gap-1">
                {!userToken ? (
                  ...
                  <li>
                    <button
                      onClick={signOut}
                      className={`block px-1 py-2 font-semibold hover:bg-[#E9522C] hover:text-white rounded-lg transition-all duration-500 ${
